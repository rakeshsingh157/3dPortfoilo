        ...
        <div className="max-w-7xl mx-auto px-6 md:px-12 flex justify-between items-center h-12">
          ...
          <div className="hidden md:flex items-center space-x-10">
            ...
            <div className="flex items-center space-x-6 border-l border-white/10 pl-10 h-6">
              ...
              {/* Hire Me Button */}
              <button onClick={() => document.getElementById('contactme')?.scrollIntoView({ behavior: 'smooth' })} className="px-5 py-1.5 border border-white/20 text-white font-mono text-[9px] uppercase tracking-[0.3em] hover:bg-white hover:text-black transition-all duration-500 rounded-sm">
                Hire Me
              </button>
            </div>
          </div>
      ...
      >
        ...
        <ul className="relative z-10 flex flex-col space-y-10 text-center">
          ...
          <li className={`pt-6 transition-all duration-500 ${isMobileMenuOpen ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`} style={{ transitionDelay: `${navLinks.length * 100}ms` }}>
            <button onClick={() => { setIsMobileMenuOpen(false); document.getElementById('contactme')?.scrollIntoView({ behavior: 'smooth' }); }} className="px-12 py-3 border border-white/20 text-white font-mono text-[10px] uppercase tracking-[0.4em] hover:bg-white hover:text-black transition-all rounded-sm">
              Hire Me
            </button>
          </li>
        </ul>
