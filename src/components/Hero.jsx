...
export default function Hero() {
    ...
    const [currentFrameIdx, setCurrentFrameIdx] = useState(130);
    ...
    const currentFrame = (index) => `/images/ezgif-frame-${(index + 1).toString().padStart(3, '0')}.png`;

    const imagesRef = useRef([]);
    // We use an object to track the frame so GSAP can animate the value smoothly
    const seqRef = useRef({ frame: startFrame });

    useEffect(() => {
        // Preload sequence
        // We only need to load frames from startFrame to frameCount
        const framesToLoad = frameCount - startFrame;
        let loadedCount = 0;

        // Fill the first part with null so indices match
        ...
        for (let i = startFrame; i < frameCount; i++) {
            ...
            img.onload = () => {
                loadedCount++;
                setLoadingProgress(Math.floor((loadedCount / framesToLoad) * 100));
                if (loadedCount === framesToLoad) setLoaded(true);
            };
            img.onerror = () => {
                loadedCount++;
                if (loadedCount === framesToLoad) setLoaded(true);
    ...
    useEffect(() => {
        ...
        // Initial Face Reveal
        bootTl.fromTo(canvas, { opacity: 0, scale: 1 }, { opacity: 1, scale: 1, duration: 1.5, ease: "power2.out" }, 0.8);
        // Slowly play initial frames immediately (starting from 130 to 140)
        bootTl.to(seqRef.current, { frame: 140, snap: "frame", duration: 1.5, ease: "power1.inOut", onUpdate: render }, 0.8);
        ...
        // User scroll takes over from whatever frame bootTl left off at, up to 284.
        const tlScroll = gsap.timeline({
            scrollTrigger: {
                trigger: containerRef.current,
                start: "top top",
                end: "+=5000", // Increased scroll distance for more hold
                scrub: 1,
                pin: true,
                anticipatePin: 1
            }
        ...
        tlScroll.to(canvas, {
            ...
            duration: 0.5,
            ease: "power2.inOut"
        }, "+=0.01"); // Wait a bit before zooming at frame 284
    ...
    const titleText = "RAKESH KUMAR SINGH";
    ...
    return (
        <div ref={containerRef} className="relative w-full h-screen bg-[#020202] overflow-hidden flex items-center justify-center font-sans tracking-wide">
            ...
            <div className="portfolio-ui absolute bottom-12 left-8 md:left-12 z-[60] flex flex-col space-y-5">
                <a href="https://github.com/rakeshsingh157" target="_blank" rel="noreferrer" className="social-icon opacity-0 text-gray-500 hover:text-white hover:drop-shadow-[0_0_8px_rgba(255,255,255,0.8)] transition-all">
                    {/* GitHub */}
                ...
                <a href="https://www.linkedin.com/in/rakesh-kumar-singh-14b17331a/" target="_blank" rel="noreferrer" className="social-icon opacity-0 text-gray-500 hover:text-white hover:drop-shadow-[0_0_8px_rgba(255,255,255,0.8)] transition-all">
                    {/* LinkedIn */}
                    ...
                    <div className="portfolio-ui absolute inset-0 z-[50] pointer-events-none flex flex-col md:flex-row justify-between items-center md:items-center px-8 md:px-[15%] lg:px-[18%] pt-32 pb-16 md:pt-0 pb-0">
                        ...
                        <div className="w-full md:w-[35%] flex flex-col items-center md:items-start text-center md:text-left mt-16 md:mt-0 md:pl-8">
                            ...
                            {currentFrameIdx >= 275 && (
                                ...
                                >
                                    <div onClick={() => document.getElementById('projects')?.scrollIntoView({ behavior: 'smooth' })} className="inline-flex items-center px-8 py-3 border border-gray-700 bg-black/50 hover:bg-black/80 hover:border-blue-500/50 transition-colors cursor-pointer rounded-sm backdrop-blur-md group">
